...
var master_controller_1 = __importDefault(require("./controllers/master.controller"));
var bodyParser = __importStar(require("body-parser"));
var router = express_1.Router();
router.use(bodyParser.urlencoded({ extended: false }));
router.use(bodyParser.json());
var master = new master_controller_1.default();
// middleware
...
router.get('/list-topup', function (req, res) {
    ...
    var token = req.headers['token'];
    jwt.verify(token, secret_1.default.secret, function (err, decoded) {
        master.list_topup(decoded.user_id, function (i) {
            res.send(i);
        });
    });
});
router.post('/confirm-topup', function (req, res) {
    ...
    var token = req.headers['token'];
    jwt.verify(token, secret_1.default.secret, function (err, decoded) {
        master.confirm_topup(req, decoded.user_id, function (i) {
            res.send(i);
        });
    });
});
